...
extern int handle_cci;
#ifdef commentttt
エントリー
    entry_pt==0：勢いが出たときに
        パーフェクトオーダー（PO)の時に、
        他にエントリーがない、CCIが
        POが上方向、＋１００以上になった：UPエントリー,exitは＋１００を下回る
        POが下方向、-１００以下になった：DNエントリー,exitはー１００を上回る
    entry_pt==2：押し戻り
        パーフェクトオーダー（PO)の時に、
        他にエントリーがない、CCIが
        POが上方向、ー１００以上になった：UPエントリー,exitは＋１００を上回る
        POが下方向、＋１００以上になった：DNエントリー,exitはー１００を下回る
...
public:
   ...
   void OnDeinit(const int reason){
      ...
      //Inpパラメータの表示
      printf("★paradouble1,2,3,4="+DoubleToString(p_allcandle.get_Inp_para_double1(),2)+
      "  ,"+DoubleToString(p_allcandle.get_Inp_para_double2(),2)+
      "  ,"+DoubleToString(p_allcandle.get_Inp_para_double3(),2)+
      "  ,"+DoubleToString(p_allcandle.get_Inp_para_double4(),2)
      );
      printf("★paraint1,2,3="+IntegerToString(p_allcandle.get_Inp_para_int1())+
      "  ,"+IntegerToString(p_allcandle.get_Inp_para_int2())+
      "  ,"+IntegerToString(p_allcandle.get_Inp_para_int3())
...
//real_point　zzzz;
   ...
   int ma_no=0;
    ...
    int count = 3;
    double cci_buff[];
    bool flag_buy_on = false;
    bool flag_buy_off = false;
    bool flag_sell_on = false;
    bool flag_sell_off = false;
    int old_idx=0;
    int new_idx=1;
      ...
      switch(hyouka_data[i].status){
         case 1:// 
            ...
            if(status_po == 0){ 
                ...
                //PO方向
//                ret = CopyBuffer(handle_cci,0,0,20,cci_buff);                //[0]古い [1]  [2]最新
                ret = CopyBuffer(handle_cci,0,0,count,cci_buff);                //[0]古い [1]  [2]最新
                if(entry_pt == 0){
                    if(status_po == 1){ 
                        //買いの信号検出
                        if(cci_buff[new_idx]>100&&cci_buff[old_idx]<=100){
                            flag_buy_on = true;ret_entry = true;
                        }
                    }else if(status_po == -1){
                        //売りの信号検出
                        if(cci_buff[new_idx]<-100&&cci_buff[old_idx]>=-100){
                            flag_sell_on = true;ret_entry = true;
                        }
                    } 
                }else if(entry_pt == 1){
                    if(status_po == 1){ 
                ...
                //if(p_MA_torimatome1.is_po_dn_edge()==true){
                //    ret_entry=true;
                //}
                //if(p_MA_torimatome1.is_po_up_edge()==true){
                //    ret_entry=true;
                //}
            }
			//エントリーできるか   
			if(ret_entry == true){
                ...
                Inp_Use_ind_lot_sikinkannri=false;//Lotは固定とするため設定 
...
//real_point　zzzz;
    ...
    for(int i = pre_start_idx_hyouka; i<hyouka_data_num ;i++){
      switch(hyouka_data[i].status){
         ...
         case 2://エントリー中
			//Exitか？
            ret = CopyBuffer(handle_cci,0,0,count,cci_buff);

            if(entry_pt == 0){
                if(hyouka_data[i].dir ==1){//買いポジションのExit判定　
                    //買いの信号終わり
                    if(cci_buff[new_idx]<=100&&cci_buff[old_idx]<100){
                        flag_exit_syori=true;exit_v=o_bid;
                    }

                }else if(hyouka_data[i].dir == -1){//売りポジションのExit判定　
                    //売りの信号終わり
                    if(cci_buff[new_idx]>=-100&&cci_buff[old_idx]<-100){
                        flag_exit_syori=true;exit_v=o_ask;
                    }
                }
            }else if(entry_pt == 1){
                if(hyouka_data[i].dir ==1){//買いポジションのExit判定　
