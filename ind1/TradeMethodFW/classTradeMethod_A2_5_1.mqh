#ifndef classTradeMethod_A2_5_1
#define classTradeMethod_A2_5_1
...
class TradeMethod_A2_5_1 :public TradeMethodbase 
{
public:
   int hyouka_data_koyuu_num;

	//--- コンストラクタとデストラクタ
	TradeMethod_A2_5_1(void){};
	TradeMethod_A2_5_1(string s,ENUM_TIMEFRAMES p,candle_data *c,allcandle *a){name = s;period = p;candle = c; p_allcandle = a;hyouka_data_koyuu_num=0;
		init_mem_hyouka_data_koyuu();
	};
	~TradeMethod_A2_5_1(void){view_kekka_youso(1);};
	//--- オブジェクトを初期化する
   void Oninit(void){}
   void OnDeinit(const int reason){
      debug_A2_5_1_tp_sl_All();
...
void debug_A2_5_1_tp_sl(real_point &A,real_point &B,real_point &C,real_point &D,real_point &E,real_point &F,real_point &G);
void TradeMethod_A2_5_1::debug_A2_5_1_tp_sl_All(void);
void debug_A2_5_1_tp_sl_idx(int idx);
    
};//end class def

void TradeMethod_A2_5_1::hyouka_kakutei(void){ // 足確定で呼ばれる想定
// 
//real_point　zzzz;
...
void TradeMethod_A2_5_1::hyouka_zig_kakutei(void){ // 足確定で呼ばれる想定
// 
//real_point　zzzz;
	real_point a,b,c,d,e,f,nn;
	double sa;//基準値からの損益価格
...
int TradeMethod_A2_5_1::chk_chg_zigdata_for_pt(int idx){// 更新し、値を変更した1（パターン成立）、未変更０、パターン成立しない２
   bool bchg=false;
	for(int i=0;i<5;i++){
...
bool    TradeMethod_A2_5_1::add_hyouka_data_koyuu(int para_refidx){
    bool ret = false;
...
//bool    TradeMethod_A2_5_1::Is_pattern(void){
//    int base_idx = candle.zigzagdata_count-1;
//	return(Is_pattern(base_idx));
//}
bool    TradeMethod_A2_5_1::Is_pattern(int base_idx){  //  基準となるzigzagcountからー１した値　　　　 int base_idx = candle.zigzagdata_count-1;
    bool ret = false;
...
bool    TradeMethod_A2_5_1::Is_pattern(void){// 成否返す。成立時　last_zigzag_E, Aの点、cn_outのABCDEの点を保持する
    bool ret = false;
    int out_dir=0;
    int chk_zigcount;
           ...
           if(   E.v >D.v && C.v > B.v && C.v > D.v &&                       D.v > B.v && E.v > C.v  &&pre_E_no!= cn_out[0].no ){
            ...
            //if(E.v >D.v && C.v > B.v && C.v > D.v && A.v>B.v&& A.v < C.v&& D.v > B.v && E.v > C.v){
                ...
                if(aaaa==3&& bkirikawari==1){
					...
					debug_A2_5_1_tp_sl(A,B,C,D,E,F,G);
...
void TradeMethod_A2_5_1::debug_A2_5_1_tp_sl(real_point &A,real_point &B,real_point &C,real_point &D,real_point &E,real_point& F,real_point &G){
...
void TradeMethod_A2_5_1::debug_A2_5_1_tp_sl_All(void){
   ...
   for(int i = 0;i< hyouka_data_koyuu_num;i++){
      if(hyouka_data[i].status ==999){
         debug_A2_5_1_tp_sl_idx(i);
      }
   }
}

void TradeMethod_A2_5_1::debug_A2_5_1_tp_sl_idx(int idx){
